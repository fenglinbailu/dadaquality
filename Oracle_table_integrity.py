...
class Oracle_table_integrity(object):
    def __init__(self,json_path,QUERY_ROW_LIMITATION,cur):                    #检测完整性
        ...
        for table in table_dict:
            result = self.check_Table_Comp(table_dict,QUERY_ROW_LIMITATION,cur,table,result)          #检测表的完整性
            count +=1
            print(count,"/",total)
        sheet.write(0,0,'表名')
        ...
        for table_name in result:
                ...
                col_num = col_num+1
        wbk.save('表3.0.xls')   
    def check_Table_Comp(self,table_dict,QUERY_ROW_LIMITATION,cur,table_name,result):
...
class Read_Table:    
    def __init__(self,from_db_flag,write_table_dict_tofile,cur, json_path):
        from_db_flag=from_db_flag
        write_table_dict_tofile=write_table_dict_tofile
        table_dict = {}
        if(from_db_flag):                           #从数据库读
            ...
            for table in table_dict:
                col_num =   table_dict[table]['col_num'] + col_num
            print("table dictionary has been read, there are ",len(table_dict), " tables and ", col_num, " columns in it")
        write_Table_Dict(table_dict,write_table_dict_tofile,json_path)
...
read_table_dict_from_db = True                #是否从数据库读表结构，False为从文件读取，True为从数据库读

write_table_dict_tofile = True               #是否要将Table_Dict写入到文件
...
table=Read_Table(read_table_dict_from_db,write_table_dict_tofile,cur,json_path)    #table_dict 操作
a=Oracle_table_integrity(json_path,QUERY_ROW_LIMITATION,cur)      
